...
int32_t recordPriorityBeforeInheritance(osSemaphoreId semaphore_id,osThreadId thread);
osPriority resumePriorityBeforeInheritance(osSemaphoreId semaphore_id,osThreadId thread);
...
osStatus osPcpSemaphoreRelease (osSemaphoreId semaphore_id)
{
	...
	semaphore_id->inuse=0;
	semaphore_id->current_used_by=NULL;
	Resource_Delete(semaphore_id);
	ThreadUsingSemaphores_Delete(semaphore_id,curr_th);	
	curr_th->priority=curr_th->opriority;
	//curr_th->priority=resumePriorityBeforeInheritance(semaphore_id,curr_th);
...
/*int32_t recordPriorityBeforeInheritance(osSemaphoreId semaphore_id,osThreadId thread)
{
	struct priority_record *p = malloc(sizeof(struct priority_record));
	if(!thread->opriority){ p->recordItem = thread->priority;p->SemaphoreId=semaphore_id; p->next=NULL; thread->spriority=p;}
		else{ p->recordItem = thread->priority;
					p->SemaphoreId=semaphore_id;
					p->next=thread->spriority;
					thread->spriority=p;
				}
	return (1);
}
osPriority resumePriorityBeforeInheritance(osSemaphoreId semaphore_id,osThreadId thread)
{	
	struct priority_record *temp;
	osPriority p;
	if(!thread->opriority) { return NULL;} //never would happen;
	for(temp=thread->spriority;temp!=NULL;temp=temp->next)
		{
			if(temp->SemaphoreId==semaphore_id)
			{
				p=temp->recordItem;
				temp->SemaphoreId=NULL;
				break;
			}
		}
